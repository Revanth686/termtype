...
class Session {
  ...
  constructor(
    ...
    this.menuBody = _menuBody;
    this.menuFooter = _menuFooter;
  ...
  static handleMenuInput(_session) {
    let session = _session;
    session.menuBody.key(["escape", "q"], function (_, key) {
      process.exit(0);
    });
    session.menuBody.key("r", function (_, key) {
      session.startSession();
    });
  }
  startSession() {
  ...
  showMenu() {
    ...
    const menuContent = `Adjusted WPM: ${adjustedWpm}\nAccuracy: ${accuracy}%\nRaw WPM:${Math.floor(wpm)}\nTime taken: ${(timeElapsed * 60).toFixed(2)}s\n${this.xWordIndices.length ? this.getMistakenWords() : ""}`;
    screen.clearRegion(0, screen.width, 0, screen.heigth);
    this.menuBody.setContent(`{center}${menuContent}{/}`);
    screen.append(this.menuBody);
    screen.append(this.menuFooter);
    this.menuBody.focus();
    screen.render();
  }
  highlightable(ind) {
...
export const main = (flags) => {
  ...
  Session.handleMenuInput(session);
};
